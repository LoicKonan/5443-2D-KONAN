...
class SpriteSheet(object):
    ...
    def __init__(self,**kwargs):
        ...
        if not os.path.isfile(self.inpath):
            print(f"{self.inpath} is not a proper file...")
            sys.exit()

        self.im = Image.open(self.inpath)
    ...
    def _saveFrame(self,x, y,sequence,quality=95):
        ...
        if self.centerit:
            frame_im = frame_im.crop(self.frame_im.getbbox())

        frame_im.save(join(self.outpath,name+'_'+sequence+".png"), quality)
...
if __name__=='__main__':
    ...
    required = ["inpath","outpath","name","rows","cols","frame_width","frame_height","direction"]

    checkRequired(required, kwargs)
    ...
    isCentered = kwargs.get("is_centered",True)
