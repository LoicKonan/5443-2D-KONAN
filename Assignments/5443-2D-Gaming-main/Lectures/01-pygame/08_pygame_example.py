...
class GameBall(object):
    ...
    def collide(self, other):
        ...
        if abs(abs(self.x) - abs(other.x)) <= self.size and abs(
            abs(self.y) - abs(other.y)
        ) <= int(self.size * 2):
...
if __name__ == "__main__":
    ...
    for i in range(max_balls):
        ...
        balls[-1].pace = (
            int(random.random() * (max_pace - min_pace)) + min_pace
        )  # give each ball different speed
    ...
    while running:
        ...
        for i in range(len(balls)):
            ...
            pygame.draw.circle(
                screen,
                (balls[i].red, balls[i].green, balls[i].blue),
                (balls[i].x, balls[i].y),
                balls[i].size,
            )
