...
def main(test: bool = False) -> None:
    ...
    settings_menu.add.text_input(
        "Some long text: ", maxwidth=19, textinput_id="long_text", input_underline="_"
    )
    settings_menu.add.text_input(
        "Password: ", maxchar=6, password=True, textinput_id="pass", input_underline="_"
    )
    ...
    settings_menu.add.selector(
        "Select difficulty:\t", items, selector_id="difficulty", default=1
    )
    settings_menu.add.selector(
    ...
    settings_menu.add.dropselect(
        "Select difficulty (drop)", items, default=1, dropselect_id="difficulty_drop"
    )
    ...
    settings_menu.add.toggle_switch(
        "First Switch", False, toggleswitch_id="first_switch"
    )
    settings_menu.add.toggle_switch(
        "Other Switch",
        True,
    ...
    rslider = settings_menu.add.range_slider(
        "Choose a number",
        50,
        (0, 100),
        1,
    ...
    settings_menu.add.range_slider(
        "How would you rate pygame-menu?",
        (7, 10),
        (1, 10),
        1,
    ...
    main_menu.add.selector(
        "Menu sounds ", [("Off", False), ("On", True)], onchange=update_menu_sound
    )
